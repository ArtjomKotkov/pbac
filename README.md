...
request = PBacRequest(
    action=Actions.READ,
    subject=user_random,  # random subject
    target=test_model,
)

print(TEST_TREE.execute(request))  # Effect.DENY

request = PBacRequest(
    action=Actions.READ,
    subject=user_admin, # admin subject
    target=test_model,
)

print(TEST_TREE.execute(request))  # Effect.PERMIT

request = PBacRequest(
    action=Actions.READ,
    subject=user_simple, # simple subject
    target=test_model,
)
