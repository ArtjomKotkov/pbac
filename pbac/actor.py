...
from const import ActorType, MODEL
...
class MetaActor(type):
    ...
    def __eq__(self, other: type[MODEL]):
...
class ActorResolver(Executable):
    ...
    def execute(self, target: MODEL, subject: MODEL):
        return self._is_correspond(target, subject)

    def _is_correspond(self, target: MODEL, subject: MODEL):
...
class ActorAndCombiner(ActorCombiner):
    def execute(self, target: MODEL, subject: MODEL) -> Any:
